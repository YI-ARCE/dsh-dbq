window.__ModuleLoader__.load({
	...
	factory: (require) => {
			...
			return createElement("div", { className: "dbqCard" },
				...
				createElement("div", { className: "dbqForm" },
					...
					createElement("select", {
						...
						createElement("option", { value: "inline" }, "inline（明文存当前 profile 配置，本地单用户）"),
		...
		function SettingsPage(props) {
			...
			const disabled = !snapshot || snapshot.status !== "ready" || readonly;
			...
			const save = (patch, okText) => {
				if (disabled) return;
				Promise.all(Object.keys(patch).map((key) => scope.set(key, patch[key]))).then((results) => {
					if (results.some((accepted) => accepted !== true)) throw new Error("配置未保存，请刷新设置后重试");
			...
			kids.push(createElement("div", { className: "dbqCard", key: "info" },
				...
				createElement("p", { className: "dbqMuted" },
					"对话中可用工具：db_connections / db_tables / db_describe / db_query（默认只读，带语句白名单与限额）。网关 dbq.exe（Windows）已随插件内置，可在下方「网关」卡查看状态或自定义路径；连接与限额保存到当前 profile 的 Cordis 配置中。"),
		...
		//#endregion
		//#region apply
		const inject = ["slots", "configForms", "sessions", "inputTriggers"];
		function apply(ctx) {
			...
			function dbqSource(trigger) {
				const CONN_SECTION = "数据库连接";
				const TABLE_SECTION = "数据库表";
				...
				const connValue = (id) => JSON.stringify({ kind: "conn", id });
				function connDesc(c) {
					const desc = [];
					desc.push(c.type === "postgres" ? "PostgreSQL" : c.type === "sqlite" ? "SQLite" : "MySQL");
					if (c.type === "sqlite") {
						if (c.database) desc.push(c.database);
					} else {
						desc.push((c.host || "127.0.0.1") + ":" + (c.port || 0) + (c.database ? "/" + c.database : ""));
					}
					if (c.label) desc.push(c.label);
					if (c.note) desc.push(c.note);
					return desc.join(" · ");
				}
				function connCandidate(c) {
					return {
						name: c.id,
						description: connDesc(c),
						icon: DbIcon,
						section: CONN_SECTION,
						value: connValue(c.id),
						drill: true,
					};
				}
				function tableCandidate(connId, t, withConn) {
					const desc = [];
					if (withConn) desc.push(connId);
					...
					return {
						...
						value: JSON.stringify({ kind: "table", ref: connId + (t.schema ? "." + t.schema : "") + "." + t.name, table: t.name }),
					};
				}
				/** 拉取一组连接的表候选行（沿用 /dbq-api/tables 约定，pattern 前缀匹配在网关侧补 %）。 */
				async function fetchTableRows(conns, pattern, req) {
					const multi = conns.length > 1;
					const lists = await Promise.all(conns.map((c) =>
						fetch("/dbq-api/tables?connection=" + encodeURIComponent(c.id) + (pattern === "" ? "" : "&pattern=" + encodeURIComponent(pattern)), { signal: req ? req.signal : undefined })
							.then((r) => r.json()).then((v) => {
								if (v === null || typeof v !== "object" || v.ok !== true) return [];
								return ((v.data && v.data.tables) || []).map((t) => tableCandidate(c.id, t, multi));
							}, () => [])
					));
					return lists.flat();
				}
				/**
				 * 两级候选源（对齐官方 @文件 源的 drill 协议）：
				 *  - 第 0 级（query 无 "."）：连接清单（本地 settings snapshot，无网络），行带 drill:true，
				 *    Tab / 回车 / 点行尾箭头进入该库（官方 MenuView 自动渲染「进入目录 · Tab」与 chevron）；
				 *  - 第 1 级（query 为 "连接." 前缀）：只查该连接的表，落定插入 #db:连接[.schema].表 记号 chip。
				 * 查询匹配不到任何连接名时回退为跨连接表名搜索，保住旧的直搜表习惯。
				 * drill 的落点是 { text, continue:true }：把 token 文本原地改写为 "@连接."，菜单保持打开，
				 * 下一级列表由管线 track -> detect -> candidates(query) 自然驱动，面包屑经 header(crumb) 提供。
				 */
				return {
					trigger,
					name: "dbq",
					...
					async candidates(session, req) {
						...
						if (dot < 0) {
							...
							const matched = ql === "" ? enabled : enabled.filter((c) => c.id.toLowerCase().includes(ql) || (c.label || "").toLowerCase().includes(ql));
							const conns = matched.map(connCandidate);
							if (conns.length > 0 || q === "") return conns.slice(0, 50);
							const rows = await fetchTableRows(enabled, q, req);
							if (req && req.signal && req.signal.aborted) return [];
							const ranked = (primitives !== null && primitives !== undefined && primitives.rankByName) ? primitives.rankByName(rows, q) : rows;
							return ranked.slice(0, 50);
						}
						const connId = q.slice(0, dot);
						const suffix = q.slice(dot + 1);
						const conn = enabled.find((c) => c.id === connId);
						if (conn === undefined) return [];
						const rows = await fetchTableRows([conn], suffix, req);
						if (req && req.signal && req.signal.aborted) return [];
						const ranked = (primitives !== null && primitives !== undefined && primitives.rankByName) ? primitives.rankByName(rows, suffix) : rows;
						return ranked.slice(0, 50);
					},
					header(_session, req) {
						// 仅 drill 到达的菜单出面包屑（对齐官方 crumbsFor：打字路径的上下文在草稿里，不欠面包屑）。
						if (!req || req.drilled !== true) return undefined;
						const q = String(req.query || "");
						const dot = q.indexOf(".");
						if (dot <= 0) return undefined;
						const connId = q.slice(0, dot);
						return [
							{ label: "数据库", value: JSON.stringify({ kind: "root" }) },
							{ label: connId, value: connValue(connId), current: true },
						];
					},
					onPick(pick) {
						const v = parseValue(pick && pick.candidate && pick.candidate.value);
						if (v === null) return undefined;
						if (v.kind === "conn") {
							// 连接没有可落定的引用语义（#db:连接 不是合法表引用）：Enter 与 Tab 都进入该库。
							const id = String(v.id || "");
							if (id === "") return { text: trigger, continue: true };
							return { text: trigger + id + ".", continue: true };
						}
						if (v.kind === "root") return { text: trigger, continue: true };
						if (v.kind === "table") {
							return { insert: {
								source: "dbq",
								ref: v.ref,
			...
			ctx.effect(() => {
				...
				// 只注册 @ 触发器：# 不在 detectTrigger 语法里（仅识别 @ 与 /），# 菜单在任何草稿改写后
				// 都会被 track 关闭，撑不起 drill；工具栏按钮改为程序化打开本源（trigger "@"）。
				const offAt = inputTriggers.registerSource(dbqSource("@"));
				return () => { offAt(); };
			...
			function DbqMenuButton(props) {
				...
				return createElement("button", {
					...
					className: "dbqAdd",
					"aria-label": "引用数据库表",
					title: "引用数据库表（先选连接，Tab/回车进入后选表，插入 #db: 记号）",
					...
					onClick: () => {
						try {
							...
							controller.toggleSource("dbq", {
								// 必须用 "@"：程序化菜单要撑起 Tab drill，依赖改写后的草稿能被 detectTrigger
								// 重新命中（# 不在检测语法里，改写后菜单会被 track 立即关闭）。
								trigger: "@",
								query: "",
								quoted: false,
								position: "inline",
