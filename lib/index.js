 ...
 *
 * 职责：
 *  1. 注册 `dsh-dbq` 设置命名空间（连接清单 + 默认限额），设置页经 settingsScope 原生读写；
 ...
 *  4. 密码来源：inline（settings.yaml 明文，本地单用户可接受）| env（网关进程环境变量）
...
export const SETTINGS_NS = 'dsh-dbq'
...
/** 设置命名空间 schema：所有字段带默认值，清空即回到出厂。 */
...
async function callGateway(settings, sub, id, subArgs) {
  const cfg = normalizeConfig(settings.get(SETTINGS_NS))
...
function registerTools(sctx, settings) {
  ...
  tools.register(makeTool(
    ...
    async (args) => {
      ...
      const data = await callGateway(settings, 'tables', args.connection, sub)
  ...
  tools.register(makeTool(
    ...
    async (args) => {
      ...
      if (args.schema) sub.push('--schema', String(args.schema))
      const data = await callGateway(settings, 'describe', args.connection, sub)
      return JSON.stringify(data)
    },
    20000,
  ))
  ...
  tools.register(makeTool(
    ...
    async (args) => {
      ...
      const data = await callGateway(settings, 'query', args.connection, sub)
      return JSON.stringify(data)
    },
    30000,
  ))
...
/**
 * 插件入口。tools/settings 是硬依赖（缺一不可注册），credentials 是可选能力
 * （只影响 credential 来源密码）；webServer 为输入框引用选择器提供表清单路由，
 * 并提供 /dbq-api/status 网关状态端点（设置页「网关」卡消费）。
 */
export function apply(ctx) {
  ctx.inject(['tools', 'settings'], (sctx) => {
    try {
      sctx.settings.register(SETTINGS_NS, SETTINGS_SCHEMA, { applies: 'live' })
    } catch (e) {
      // 命名空间已注册（热重载/重复加载）——继续注册工具。
      console.error('dsh-dbq: settings namespace register skipped:', e && e.message)
    }
    registerTools(sctx, sctx.settings)
  })
        ...
        handler: async (req, res) => {
          ...
          try {
            ...
            if (url.pathname === '/dbq-api/status') {
              const cfg = normalizeConfig(wctx.settings.get(SETTINGS_NS))
              const gw = resolveGateway(cfg)
            ...
            if (url.pathname === '/dbq-api/tables') {
              ...
              const data = await callGateway(wctx.settings, 'tables', connection, sub)
